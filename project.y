...
void codegen_algebric()
{
	...
	node *left = NULL;
	node *right = NULL;
	a = mknode(left, right, st[top-2]);
	b = mknode(left, right, st[top]);
	c = mknode(a, b, st[top-1]);
  	fprintf(f1,"%s\t=\t%s\t%s\t%s\n",tempo,st[top-2],st[top-1],st[top]);
	printf("\n");
  	top-=2;
 	strcpy(st[top],tempo);
...
void printtree(node *tree) // for algrebric expressions with 2 adds or subtract or multiplicat
{
  static int  i = 0;
  if(i == 0)
  	fprintf(f2, "Basic AST for arthimetic operators\n");
  fprintf(f2, "For the expression %s %s %s the AST is :\n", tree->left->token, tree->token, tree->right->token);
  fprintf(f2,"\n");
  fprintf(f2,"	     %s\n", tree->token);
  fprintf(f2,"	   /   \\\n");
